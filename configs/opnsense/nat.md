...
OPNsense fait du NAT sortant (masquerading) pour permettre aux réseaux internes d'accéder à Internet via l'interface WAN (`10.0.99.254`).

Le PC Windows fait ensuite un second NAT depuis `10.0.99.1` vers Internet via Wi-Fi.

```
VM interne (10.0.30.x)
  → OPNsense MASQUERADE (10.0.30.x → 10.0.99.254)
  → PC Windows MASQUERADE (10.0.99.254 → IP Wi-Fi)
  → Internet
```
...
|---|---|---|---|---|
| WAN | `10.0.10.0/24` | * | Interface WAN (`10.0.99.254`) | MGMT → Internet |
| WAN | `10.0.20.0/24` | * | Interface WAN (`10.0.99.254`) | DMZ → Internet |
| WAN | `10.0.30.0/24` | * | Interface WAN (`10.0.99.254`) | SRV → Internet |
...
Source      : 10.0.10.0/24
Destination : any
Translation : Interface address (10.0.99.254)
